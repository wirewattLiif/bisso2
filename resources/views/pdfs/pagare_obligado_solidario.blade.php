...
<script type="text/php">
    ...
    <p style="text-align: justify;">
        ...
        @endphp
        <b>POR VALOR RECIBIDO</b>, el suscrito

        @if( $solicitud->cliente->persona_tipo == 'moral')
            <b class="upper">{{ $solicitud->cliente->nombre_empresa }}</b> (el <span class="underline">“Suscriptor”</span>)
        @else
            <b class="upper">{{ $solicitud->cliente->nombre . ' ' . $solicitud->cliente->apellido_paterno . ' ' . $solicitud->cliente->apellido_materno}}</b> (el <span class="underline">“Suscriptor”</span>)
        @endif



        y <b>{{ @$solicitud->cliente->obligado_solidario->nombre }} {{ @$solicitud->cliente->obligado_solidario->apellido_paterno }} {{ @$solicitud->cliente->obligado_solidario->apellido_materno }} </b> (el <span class="underline">“Obligado Solidario”</span>) por medio de este pagaré (el
        <span class="underline">“Pagaré”</span>) prometen pagar incondicionalmente a la orden de <b class="underline">{{ @$solicitud->razon_social->razon_social }}</b> (el <span class="underline">“Tenedor”</span>), la suma principal de
        <b>${{ number_format($periodos['suma_amortizaciones'],2) }} </b>
        <b>({{ $total_letras }} PESOS {{ $total_centavos }}/100  Moneda Nacional)</b>

        ya con el Impuesto al Valor Agregado correspondiente (el <span class="underline">“Principal”</span>) pagadera a más tardar el día
        <b>{{  $fecha }} </b> (la <span class="underline">“Fecha de Vencimiento”</span>), mediante <b>{{ count($periodos) -1 }}</b>

        (<b>{{ NumeroALetras::convertir((count($periodos) -1))}}</b>) amortizaciones mensuales y
        consecutivas (cada una, un <span class="underline">“Monto de Amortización en Pesos”</span>) en cada fecha de pago indicada en la tabla de
        amortizaciones a continuación (cada una, una <span class="underline">“Fecha de Pago”</span>) en el domicilio del Tenedor ubicado en
        ...
        por los Montos de Amortización en Pesos y en cada Fecha de Pago que se indican a continuación (las
        <span class="underline">“Amortizaciones”</span>):
    </p>
    ...
    <p style="text-align: justify;">
        En el supuesto que el Suscriptor o el Obligado Solidario incumplan con cualquiera de las amortizaciones a las que
        se obligan conforme a los términos del presente Pagaré, tendrá como consecuencia que todos los pagos
        pendientes y los subsecuentes se darán por vencidos en forma anticipada de manera automática, siendo exigible el
        monto total que resulte insoluto del Principal, más los Intereses Moratorios que correspondan.
        <br><br>
        El Suscriptor y el Obligado Solidario se obligan a pagar intereses moratorios sobre el Principal vencido y no
        pagado, mismos que se generarán desde el día siguiente de cada Fecha de Pago, hasta el día en que dicho Principal
        vencido y no pagado quede totalmente pagado, a razón de una tasa anual del {{ @$solicitud->razon_social->interes_moratorio }}% ({{ NumeroALetras::convertir(@$solicitud->razon_social->interes_moratorio) }} por ciento) (los
        <span class="underline">“Intereses Moratorios”</span>). Los Intereses Moratorios se calcularán en base a un año de 360 (trescientos sesenta) días
        y deberán ser pagados en cada Fecha de Pago correspondiente a cada Monto de Amortización en Pesos y sus
        accesorios.
        <br><br>
        ...
        Fecha de Pago, en Pesos, moneda en curso legal en los Estados Unidos Mexicanos (“Pesos”), en fondos
        ...
        el Suscriptor o el Obligado Solidario deba hacer un pago de Principal o cubrir un Monto de Amortización en
        Pesos conforme a este Pagaré.
        <br><br>
        ...
        <br><br>
        El importe Principal de este Pagaré y los intereses sobre el mismo serán pagados por el Suscriptor o el Obligado
        Solidario libres y sin deducción alguna por concepto de cualesquiera impuestos, tributos, contribuciones,
        deducciones, cargos, retenciones, cualesquiera intereses, recargos, multas, sanciones y otros gravámenes fiscales
        presentes o futuros de cualquier clase respecto de los mismos.
        <br><br>
        ...
        de las partes. Para cualquier acción o procedimiento derivado de o relativo al presente Pagaré, el Suscriptor y el
        Obligado Solidario, se someten expresamente a la jurisdicción de los tribunales competentes de la Ciudad de
        Monterrey, Nuevo León, y renuncia expresamente por lo tanto, a cualquier otra jurisdicción a la que pudiera tener
        derecho, incluyendo pero no limitado a, jurisdicción por razón de su domicilio presente o futuro o por razón del
        lugar de pago de este Pagaré o por cualquier otra razón.
        <br><br>
        Por el presente Pagaré, el Suscriptor y el Obligado Solidario renuncian a cualquier diligencia de presentación,
        requerimiento, protesto y a toda notificación en relación con este Pagaré.
        <br><br>
        ...
        <br><br>
        El Suscriptor y el Obligado Solidario por medio del presente Pagaré, prometen incondicionalmente pagar los
        gastos que implique el cobro de este Pagaré y los honorarios de los abogados que intervengan en el mismo cobro
        en caso de que haya incumplimiento de pago de este Pagaré.
        <br><br>
        Este Pagaré se suscribe en la Ciudad de Monterrey, Estado de Nuevo León, Estados Unidos Mexicanos, el día <b>{{ \Carbon\Carbon::now()->formatLocalized('%d de %B de %Y') }}</b>

    </p>

    <div class="page-break"></div>

    <h3 style="text-align: center" class="upper">El suscriptor</h3>
    ...
    <p style="text-align: center;width: 350px;margin: 0 auto;font-weight: bold" class="upper">
        @if( $solicitud->cliente->persona_tipo == 'moral')
            ...
            <br>
            Domicilio: {{ @$solicitud->cliente->domicilio_empresa->calle  }} Ext:{{ @$solicitud->cliente->domicilio_empresa->numero_ext  }} Int:{{ @$solicitud->cliente->domicilio_empresa->numero_int  }}
            ...
            C.P: {{ @$solicitud->cliente->domicilio_empresa->cp }}
        @else
    ...
    <h3 style="text-align: center;margin-top:65px" class="upper">El obligado solidario:</h3>
    <hr style="width: 300px;margin-top: 80px">
    <p style="text-align: center;width: 350px;margin: 0 auto;font-weight: bold" class="upper">
        {{ @$solicitud->cliente->obligado_solidario->nombre }} {{ @$solicitud->cliente->obligado_solidario->apellido_paterno }} {{ @$solicitud->cliente->obligado_solidario->apellido_materno }}
        <br>
