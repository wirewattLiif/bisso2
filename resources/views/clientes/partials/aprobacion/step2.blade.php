<div class="row">
    ...
    <div class="col-md-1">
        <select id="f_dia" class="form-control input_underline">
            @php
                $dia = "";
                if ( !is_null(Auth::user()->cliente->fecha_nacimiento)){
                    $dia = date('d',strtotime(Auth::user()->cliente->fecha_nacimiento));
                }
            @endphp
    ...
    <div class="col-md-2">
        <select id="f_mes" class="form-control input_underline">
            @php
                $mes = "";
                if ( !is_null(Auth::user()->cliente->fecha_nacimiento)){
                    $mes = date('m',strtotime(Auth::user()->cliente->fecha_nacimiento));
                }
            @endphp
    ...
    <div class="col-md-1">
        <select id="f_anio" class="form-control input_underline">
            @php
                $anio  = "";
                if ( !is_null(Auth::user()->cliente->fecha_nacimiento)){
                    $anio = date('Y',strtotime(Auth::user()->cliente->fecha_nacimiento));
                }
            @endphp
    ...
    <div class="col-md-5">
        ...
        <select name="estado_nacimiento_id" id="estado_nacimiento_id" class="form-control input_underline">
            ...
            @foreach($estados as $k => $v)
                <option value="{{ $k }}" {{ (\Illuminate\Support\Facades\Auth::user()->cliente->estado_nacimiento_id == $k)?'selected':'' }}>{{ $v }}</option>
            @endforeach
        </select>
    </div>
</div>
...
<div class="row">
    <div class="col-md-5">
        <p>RFC</p>
        <input id="rfc" maxlength="13" placeholder="{{ __('RFC') }}" type="text" class="input_underline form-control{{ $errors->has('rfc') ? ' is-invalid' : '' }}" value="{{ \Illuminate\Support\Facades\Auth::user()->cliente->rfc }}">
    </div>
</div>
...
<div class="row">
    <div class="col-md-5">
        <p>CURP</p>
        <input id="curp" maxlength="18" placeholder="{{ __('CURP') }}" type="text" class="input_underline form-control{{ $errors->has('curp') ? ' is-invalid' : '' }}" value="{{ \Illuminate\Support\Facades\Auth::user()->cliente->curp }}">
    </div>
</div>
...
<script>
    ...
    function setSexo(){
        var sexo = '{{ (!empty(Auth::user()->cliente->sexo))?Auth::user()->cliente->sexo:'' }}';
        if(sexo != ''){
            $('.btnGenero').removeClass('btn-success').addClass('btn-default').addClass('btn-outline');
